...
watch(() => auth.organization.value?.id, (id) => {
  selectedOrgId.value = id || ''
}, { immediate: true })

async function handleOrgSwitch() {
  if (!selectedOrgId.value) return
  await auth.switchOrganization(selectedOrgId.value)
}
  ...
  <div class="layout-app" :class="{ 'layout-app--collapsed': sidebarCollapsed }">
    <aside class="layout-app__sidebar">
      ...
      <nav class="layout-app__nav">
        ...
        <NuxtLink v-if="screenshots" to="/app/screenshots" class="nav-link" :class="{ 'nav-link--collapsed': sidebarCollapsed }">
    ...
    <div class="layout-app__main">
      <header class="layout-app__header">
        ...
        <div class="layout-app__user">
          <select
            v-if="!auth.isLoading.value && auth.organizations.value.length > 1"
            v-model="selectedOrgId"
            class="layout-app__org-select"
            aria-label="Select organization"
            @change="handleOrgSwitch"
          >
            <option v-for="org in auth.organizations.value" :key="org.id" :value="org.id">{{ org.name }}</option>
          </select>
          <ClientOnly><UiThemeToggle /></ClientOnly>
          <span v-if="!auth.isLoading.value" class="layout-app__user-name">{{ auth.user.value?.name }}</span>
