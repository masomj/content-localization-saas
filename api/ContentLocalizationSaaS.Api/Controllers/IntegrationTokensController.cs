...
public sealed record CreateApiTokenRequest(string Name, string Scope);
...
public sealed class IntegrationTokensController(AppDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> List(CancellationToken cancellationToken)
    {
        ...
        return Ok(rows.Select(x => new { x.Id, x.Name, x.Scope, x.IsRevoked, x.CreatedUtc, x.RevokedUtc }));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CreateApiTokenRequest request, CancellationToken cancellationToken)
    {
        ...
        return Ok(new { token = rawToken, row.Id, row.Name, row.Scope });
    }
